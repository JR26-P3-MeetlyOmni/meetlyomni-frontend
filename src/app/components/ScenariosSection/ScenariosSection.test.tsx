...
describe('ScenariosSection', () => {
  ...
  it('renders descriptions inside each card', () => {
    render(<ScenariosSection {...mockProps} />);
    expect(screen.getByText('Interactive quiz & Lucky draw to enhance team cohesion!')).toBeInTheDocument();
    expect(screen.getByText('Live sweepstakes enhance interaction and let customers remember your brand!')).toBeInTheDocument();
    expect(screen.getByText('Gamified Q&A allows students to absorb knowledge faster and improve the training effect!')).toBeInTheDocument();
    expect(screen.getByText('Viewers scan the code to enter the interactive Q&A & Sweepstakes to improve live stream retention!')).toBeInTheDocument();
  });
